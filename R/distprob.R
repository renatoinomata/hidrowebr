  ...
  for (i in unique(dados$Est)){
    ...
    if(any(dist == "lgamma3")){
      ...
      if(!assertthat::is.error(fit)){
        ...
        KS <- NA
        estimado <- NA
        lgamma3 <- list(coefs, KS, estimado)
      }
      names(lgamma3) <- c("coefs", "KS", "estimado")
      distEst[["lgamma3"]] <- lgamma3
      nomesDist <- c(nomesDist, "lgamma3")
    }
    names(distEst) <- nomesDist

    # Lista com todas as estações
    distribuicao[[i]] <- distEst
  }
  names(distribuicao) <- as.character(unique(dados$Est))
...
print.distprob.hidrowebr <- function(x, ...){
  ...
  for (j in 1:numEst){
    ...
    for(i in 1:numDist){
      KS <- c(KS, x[[j]][[i]]$KS)
      estimado <- c(estimado, x[[j]][[i]]$estimado)
    }
    names(KS) <- namesDist
